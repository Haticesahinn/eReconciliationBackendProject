...
namespace Business.Concrete
{
    public class CurrencyAccountManager : ICurrencyAccountService
    {
        ...
        public IResult Delete(CurrencyAccount currencyAccount)
        {
            ...
            if (result == false)
            {
                return new ErrorResult(Messages.AccountHaveReconciliations);
            }
        ...
        {
            return new SuccessDataResult<List<CurrencyAccount>>(_currencyAccountDal.GetList(p => p.CompanyId == companyId).OrderBy(p => p.Name).ToList());
        }
